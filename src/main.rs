...
fn main() -> Result<(), Box<dyn Error>> {
	...
	if args.help {
		...
		println!();
		println!("OPTIONS:");
		println!("    -g, --global           Run in global mode, even when in a crate");
		println!(
	...
	let data_dir = args
		...
		.unwrap_or(dirs::data_dir().expect(
			"No data directory found on this system. Run again by providing a writable data directory with --data-dir",
		))
		.join("rspl");
...
fn parse_args() -> Result<Args, pico_args::Error> {
	...
	let global_override = args.contains(["-g", "--global"]);
